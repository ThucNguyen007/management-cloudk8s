...
const elasticClient = new elasticsearch.Client({
    ...
    // needed when we are running Elastic, we need to set the port
    //hosts: [ envProps.elasticHost ], // used when Elastic is running as a service in AWS
    log: 'trace'
});

const TODO_SEARCH_INDEX_NAME = "products";
const TODO_SEARCH_INDEX_TYPE = "product";
    ...
    if (error) {
        ...
        const todoIndexExists = elasticClient.indices.exists({
            ...
            if (error) {
                ...
                console.log('Product index exists in Elasticsearch');
            }
        });
        ...
        if (!todoIndexExists) {
            ...
            elasticClient.indices.create({
                ...
                if (error) {
                    console.log('Could not create Product index in Elasticsearch: ' + error);
                } else {
                    console.log('Created Product index in Elasticsearch');
                }
            });
        }
    }
...
rabbitmq.connect(envProps.rabbitmqUrl, function(err, connection) {
    connection.createChannel(function(err, channel) {
        ...
        channel.consume(searchIngestionQueue, function(msg) {
            ...
            console.log("Received '%s' product", todoTitle);
            ...
            elasticClient.index({
                ...
                if (err) {
                    ...
                    console.log('Added Product: [' + todoTitle + '] to Elasticsearch Index');
                }
            });
